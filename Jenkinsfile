pipeline {
    ...
    parameters {
        ...
        */
        choice(
            name: 'DEPLOY_TARGET', 
            choices: ['tenant-f', 'tenant-a', 'tenant-b', 'tenant-c', 'tenant-d', 'tenant-e','tenant-test'], 
        ...
        stage('Deploy to S3') {
            steps {
                script {
                    ...
                    def allTenants = ['tenant-a', 'tenant-b', 'tenant-c', 'tenant-d', 'tenant-e','tenant-f','tenant-test']
                    def targets = []
